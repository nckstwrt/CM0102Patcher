    ...
    static class Program
    {
        ...
        static void Main()
        {
            ...
            */
            ...
            /*
            HistoryLoader hl = new HistoryLoader();
            hl.Load(@"C:\ChampMan\Championship Manager 0102\TestQuick\Orig\Data\index.dat", false);
            var clubCount = hl.club.Count;
            var euroClubs = hl.club.Where(x => x.EuroFlag > 0).ToList();
            var clubCompCount = hl.club_comp.Count;
            Console.WriteLine();
            */
            /*
            HistoryLoader hl = new HistoryLoader();
            ...
            foreach (var italyLeague in allItalianLeagues)
            {
                var leagueTeams = allItalianClubs.Where(x => x.Division == italyLeague.ID).OrderBy(x => x.Name.ReadString()).ToList();
                if (leagueTeams.Count > 0)
                {
                    Console.WriteLine("# " + italyLeague.Name.ReadString() + " (Count: " + leagueTeams.Count + ")");
                    foreach (var team in leagueTeams)
                        Console.WriteLine("CHANGECLUBLASTDIVISION \"" + team.Name.ReadString() + "\" \"" + italyLeague.Name.ReadString() + "\"");
                    Console.WriteLine();
                }
            }
