...
namespace CM0102Patcher
{
    public class SuperStarMaker
    {
        public int MakeSuperStar(List<TStaff> staff, List<TPlayer> players, byte[] staffData, byte[] playerData, DateTime? gameDate = null, int age = -1)
        {
            ...
            if (gameDate != null && age != -1)
            {
                ...
                if (birthday.Month > gameDate.Value.Month || (birthday.Month == gameDate.Value.Month && birthday.Day >= gameDate.Value.Day)) // Force the age to be correct
                    ageMod = 1;
                birthday = new DateTime(gameDate.Value.AddYears(-(age + ageMod)).Year, birthday.Month, birthday.Day);
        ...
        public int MakeTHERichardMolloy(List<TStaff> staff, List<TPlayer> players, DateTime? gameDate, int age, List<string> firstNames, List<string> secondNames)
        {
            int molly = 0;
            var richardIdx = firstNames.IndexOf("Richard");
            var molloyIdx = secondNames.IndexOf("Molloy");
            var maxim = stars.First(x => x.name == "Maxim Tsigalko");

            if (richardIdx != -1 && molloyIdx != -1)
            {
                molly = MakeSuperStar(staff, players, maxim.staffBytes, maxim.playerBytes, gameDate, age);
        ...
        public void OutputPlayer(TStaff thePlayer, List<TPlayer> players, List<string> firstNames, List<string> secondNames, DateTime gameDate)
        {
            ...
            Console.WriteLine("\t\tstaffBytes = new byte[] {{ {0} }},", ByteArrayToString(staffBytes, 86, 8));
            Console.WriteLine("\t\tplayerBytes = new byte[] {{ {0} }},", ByteArrayToString(playerBytes));
            Console.WriteLine("\t\tage = {0}", thePlayer.YearOfBirth);
            Console.WriteLine("\t\t},");
        }
        ...
        {
            public string name;
            public byte[] staffBytes;
            public byte[] playerBytes;
            public int age;
        }

        // https://champman0102.co.uk/showthread.php?t=631
        public Star[] stars = new Star[] {
            new Star {
                name = "Maxim Tsigalko",
                staffBytes = new byte[] { 0x0E, 0x09, 0x12, 0x10, 0x0F, 0x08, 0x09, 0x07 },
                playerBytes = new byte[] { 0x93, 0x37, 0x00, 0x00, 0x00, 0x50, 0x00, 0x6F, 0x00, 0x82, 0x14, 0x82, 0x14, 0xD6, 0x06, 0x00, 0x00, 0x00, 0x00, 0x0A, 0x14, 0x14, 0x00, 0x0A, 0x00, 0x14, 0x0A, 0x11, 0x11, 0x13, 0xD1, 0x0E, 0x0F, 0x0F, 0x08, 0xBE, 0xD6, 0x08, 0xB4, 0x33, 0x10, 0x06, 0x88, 0xD1, 0x12, 0x02, 0x12, 0x05, 0x10, 0xB9, 0xCA, 0x2E, 0x12, 0x04, 0x0F, 0xCC, 0xE0, 0xBE, 0xD1, 0x14, 0x0E, 0x0E, 0xCA, 0x0C, 0x0D, 0x09, 0x0A, 0xFE, 0x0D, 0x0A },
                age = 18
            },
            new Star {
                name = "Cherno Samba",
                staffBytes = new byte[] { 0x08, 0x14, 0x11, 0x0A, 0x0D, 0x13, 0x0D, 0x0C },
                playerBytes = new byte[] { 0x03, 0x16, 0x00, 0x00, 0x00, 0x32, 0x00, 0xB5, 0x00, 0xA6, 0x0E, 0xA6, 0x0E, 0xC4, 0x09, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x14, 0x01, 0x01, 0x01, 0x14, 0x01, 0x10, 0x0C, 0x0D, 0xCA, 0x0F, 0x0B, 0x09, 0x09, 0xB5, 0xBB, 0x03, 0xF2, 0x00, 0x12, 0x09, 0x88, 0xFB, 0x0B, 0x05, 0x10, 0x06, 0x14, 0xF2, 0xB5, 0xED, 0x0F, 0x33, 0x10, 0xE3, 0x4B, 0xCA, 0xD9, 0x14, 0x0C, 0x0E, 0xBA, 0x11, 0x0B, 0xFC, 0x09, 0xC0, 0x0C, 0x0A },
                age = 15
            },
            new Star {
                name = "Anastasios Skalidis",
                staffBytes = new byte[] { 0x13, 0x12, 0x14, 0x13, 0x14, 0x13, 0x14, 0x10 },
                playerBytes = new byte[] { 0xC8, 0x59, 0x00, 0x00, 0x09, 0x32, 0x00, 0x8A, 0x00, 0xC4, 0x09, 0xC4, 0x09, 0x32, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x14, 0x00, 0x00, 0x00, 0x14, 0x00, 0x11, 0x0F, 0x0C, 0x03, 0x14, 0x14, 0x14, 0x02, 0xB8, 0x03, 0x10, 0x08, 0x50, 0x14, 0x13, 0x88, 0x11, 0x14, 0x13, 0x14, 0x12, 0x14, 0x11, 0xAE, 0x08, 0x12, 0x4B, 0x11, 0xAE, 0x11, 0xAE, 0xAE, 0x13, 0x12, 0x14, 0xAE, 0x01, 0x11, 0xF1, 0x01, 0xAE, 0x10, 0x0A },
                age = 19
            },
            new Star {
                name = "Alexandros Papadopoulos",
                staffBytes = new byte[] { 0x13, 0x12, 0x14, 0x13, 0x12, 0x14, 0x09, 0x0E },
                playerBytes = new byte[] { 0x7F, 0x3B, 0x00, 0x00, 0x10, 0x50, 0x00, 0x8B, 0x00, 0xA0, 0x0F, 0xA0, 0x0F, 0x32, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x14, 0x00, 0x00, 0x00, 0x14, 0x00, 0x11, 0x12, 0x0C, 0xF1, 0x0E, 0x13, 0x14, 0x02, 0xB6, 0x0A, 0x13, 0xFB, 0x28, 0x12, 0x01, 0x88, 0xD8, 0x12, 0x01, 0x0A, 0x11, 0x14, 0xB6, 0xB6, 0x0F, 0x14, 0x3E, 0x10, 0xFB, 0x0F, 0xB6, 0xB6, 0x13, 0x13, 0x0D, 0x9E, 0x13, 0x0F, 0xE9, 0x01, 0xFB, 0x12, 0x0A },
                age = 16
            },
            new Star {
                name = "Fernando José Torres",
                staffBytes = new byte[] { 0x11, 0x14, 0x14, 0x12, 0x0C, 0x10, 0x11, 0x10 },
                playerBytes = new byte[] { 0x6E, 0x24, 0x00, 0x00, 0x09, 0x8A, 0x00, 0x97, 0x00, 0x5E, 0x1A, 0x5E, 0x1A, 0x64, 0x19, 0x01, 0x01, 0x01, 0x01, 0x05, 0x0A, 0x14, 0x01, 0x0A, 0x0A, 0x14, 0x09, 0x0F, 0x0A, 0x10, 0xD6, 0x09, 0x0D, 0x10, 0x09, 0xDB, 0xFE, 0x0A, 0xF9, 0xFE, 0x14, 0x08, 0x88, 0xE5, 0x11, 0x06, 0x09, 0x0D, 0x08, 0xDB, 0xD4, 0x08, 0x07, 0x0F, 0x10, 0xE0, 0xF4, 0xD4, 0xCA, 0x14, 0x0E, 0x08, 0xD1, 0x0A, 0x0F, 0xCF, 0x0A, 0xE5, 0x0D, 0x0A },
                age = 17
            },
            new Star {
                name = "Mike Duff",
                staffBytes = new byte[] { 0x0C, 0x12, 0x14, 0x0F, 0x0F, 0x12, 0x10, 0x0F },
                playerBytes = new byte[] { 0xA9, 0x2E, 0x00, 0x00, 0x06, 0x73, 0x00, 0x73, 0x00, 0xB8, 0x0B, 0xB8, 0x0B, 0xEE, 0x02, 0x01, 0x01, 0x14, 0x0A, 0x0A, 0x01, 0x01, 0x0F, 0x12, 0x01, 0x14, 0x01, 0x12, 0x04, 0x12, 0xC6, 0x11, 0x10, 0x0F, 0x0E, 0xDC, 0xDC, 0x03, 0xCB, 0xB7, 0x10, 0x0B, 0x88, 0xDC, 0x11, 0x04, 0x12, 0x0B, 0x11, 0xD6, 0xE1, 0xE1, 0x14, 0x05, 0x12, 0xD7, 0xD6, 0xDC, 0xCB, 0x14, 0x14, 0x0C, 0x23, 0x12, 0x10, 0x31, 0x10, 0xCB, 0x12, 0x0A },
                age = 23
            },
            new Star {
                name = "Mark Kerr",
                staffBytes = new byte[] { 0x0C, 0x14, 0x14, 0x0A, 0x0D, 0x14, 0x0A, 0x14 },
                playerBytes = new byte[] { 0x00, 0x30, 0x00, 0x00, 0x00, 0x7B, 0x00, 0x7B, 0x00, 0x8E, 0x12, 0x8E, 0x12, 0xC4, 0x09, 0x01, 0x01, 0x01, 0x01, 0x14, 0x01, 0x01, 0x00, 0x0A, 0x01, 0x14, 0x00, 0x11, 0x0D, 0x11, 0xE0, 0x12, 0x10, 0x13, 0x0E, 0xE5, 0xD6, 0x06, 0xE0, 0xC4, 0x0D, 0x0D, 0x88, 0xC8, 0x10, 0x0A, 0x0E, 0x0B, 0x14, 0xD6, 0xC8, 0x26, 0x11, 0x03, 0x0C, 0xEA, 0xE0, 0xC4, 0xC3, 0x14, 0x12, 0x0E, 0xE5, 0x14, 0x12, 0x03, 0x0C, 0xDB, 0x13, 0x0A },
                age = 19
            },
            new Star {
                name = "Hugo Pinheiro",
                staffBytes = new byte[] { 0x0C, 0x10, 0x0E, 0x0C, 0x09, 0x0C, 0x0E, 0x0C },
                playerBytes = new byte[] { 0xB1, 0x68, 0x00, 0x00, 0x00, 0x5A, 0x00, 0x66, 0x00, 0xE8, 0x03, 0xE8, 0x03, 0x32, 0x00, 0x14, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x09, 0x0E, 0x0E, 0xF2, 0x0E, 0x13, 0x0E, 0x01, 0xCD, 0x01, 0x01, 0xC9, 0xCE, 0x01, 0x09, 0x17, 0xD7, 0x0C, 0x01, 0x10, 0x0C, 0x07, 0xE6, 0xCD, 0xCD, 0x0E, 0x0F, 0x0F, 0xD3, 0xD7, 0x0B, 0xEA, 0x14, 0x0E, 0x0F, 0xD3, 0x0C, 0x0E, 0xD8, 0x03, 0xDC, 0x0F, 0x0A },
                age = 20
            },
            new Star {
                name = "Franco Costanzo",
                staffBytes = new byte[] { 0x0F, 0x0F, 0x14, 0x0D, 0x10, 0x07, 0x04, 0x0F },
                playerBytes = new byte[] { 0xF0, 0x5C, 0x00, 0x00, 0x00, 0x82, 0x00, 0x82, 0x00, 0x70, 0x17, 0x70, 0x17, 0xB8, 0x0B, 0x14, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x14, 0x0E, 0x14, 0x06, 0x10, 0x0C, 0x14, 0x03, 0xC4, 0xE8, 0x04, 0xBC, 0xC8, 0x04, 0x02, 0x00, 0xBC, 0x10, 0x12, 0x0F, 0x08, 0x07, 0xC8, 0xB2, 0xC8, 0x10, 0x26, 0x0F, 0xCF, 0xC8, 0x1A, 0x00, 0x14, 0x12, 0x13, 0xDE, 0x0F, 0x07, 0xFE, 0x01, 0xC5, 0x11, 0x0A },
                age = 20
            },
            new Star {
                name = "Dionisis Chiotis",
                staffBytes = new byte[] { 0x14, 0x12, 0x13, 0x13, 0x11, 0x0F, 0x07, 0x0A },
                playerBytes = new byte[] { 0x23, 0x59, 0x00, 0x00, 0x16, 0x7D, 0x00, 0x82, 0x00, 0x6A, 0x18, 0x6A, 0x18, 0x4E, 0x0C, 0x14, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0F, 0x05, 0x13, 0x1D, 0x0C, 0x14, 0x12, 0x0A, 0xCE, 0x09, 0x08, 0xCE, 0xCE, 0x05, 0x05, 0x00, 0xFB, 0x0B, 0x03, 0x13, 0x08, 0x05, 0xCE, 0xCE, 0xCE, 0x11, 0x0D, 0x12, 0xFB, 0xE2, 0x09, 0x00, 0x14, 0x11, 0x12, 0xE2, 0x0C, 0x02, 0xD6, 0x01, 0xCE, 0x14, 0x0A },
                age = 24
            },
            new Star {
                name = "Taribo West",
                staffBytes = new byte[] { 0x0D, 0x0E, 0x13, 0x0A, 0x12, 0x0A, 0x0E, 0x11 },
                playerBytes = new byte[] { 0x01, 0x0F, 0x00, 0x00, 0x00, 0x86, 0x00, 0xB4, 0x00, 0x76, 0x16, 0x76, 0x16, 0x76, 0x16, 0x01, 0x0D, 0x14, 0x08, 0x01, 0x01, 0x01, 0x05, 0x0D, 0x11, 0x14, 0x01, 0x0F, 0x14, 0x09, 0xD4, 0x12, 0x14, 0x0C, 0x08, 0xDE, 0xDE, 0x14, 0xED, 0xDD, 0x0B, 0x04, 0x88, 0xF7, 0x10, 0x0C, 0x11, 0x0E, 0x0C, 0xDE, 0x06, 0xE3, 0x0E, 0xD7, 0x10, 0xD4, 0xC3, 0xF2, 0xC7, 0x14, 0x13, 0x14, 0x01, 0x0F, 0x07, 0x27, 0x0E, 0xC3, 0x12, 0x0A },
                age = 27
            },
            new Star {
                name = "Kim Källström",
                staffBytes = new byte[] { 0x0C, 0x13, 0x12, 0x0D, 0x12, 0x0E, 0x0D, 0x0B },
                playerBytes = new byte[] { 0x49, 0x6F, 0x00, 0x00, 0x0C, 0x8E, 0x00, 0x8E, 0x00, 0x46, 0x1E, 0x46, 0x1E, 0xA0, 0x0F, 0x01, 0x01, 0x01, 0x0F, 0x14, 0x14, 0x07, 0x01, 0x01, 0x08, 0x14, 0x0F, 0x07, 0x0C, 0x08, 0xE7, 0x0E, 0x10, 0x10, 0x0D, 0xE7, 0xEC, 0x07, 0xDD, 0xE2, 0x0D, 0x12, 0x88, 0xD4, 0x0C, 0x05, 0x0A, 0x0B, 0x14, 0x00, 0xDC, 0xDD, 0x11, 0xDA, 0x0A, 0xFB, 0xE7, 0xDC, 0xDC, 0x03, 0x0F, 0x08, 0xD6, 0x0D, 0x0F, 0x0C, 0x08, 0xFB, 0x11, 0x0A },
                age = 18
            },
            new Star {
                name = "Eldar Hadzimehmedovic",
                staffBytes = new byte[] { 0x04, 0x05, 0x0A, 0x11, 0x0A, 0x09, 0x01, 0x08 },
                playerBytes = new byte[] { 0x83, 0x65, 0x00, 0x00, 0x0F, 0x66, 0x00, 0x9C, 0x00, 0x88, 0x13, 0x88, 0x13, 0x96, 0x00, 0x01, 0x01, 0x01, 0x01, 0x0A, 0x14, 0x14, 0x00, 0x11, 0x00, 0x14, 0x00, 0x0C, 0x08, 0x0C, 0xD8, 0x0D, 0x0E, 0x0B, 0x0D, 0xD8, 0xF1, 0x0A, 0xE7, 0x16, 0x0E, 0x0A, 0x88, 0xD5, 0x09, 0x09, 0x09, 0x09, 0x01, 0xE2, 0xD0, 0xEC, 0x0A, 0x03, 0x0D, 0xF6, 0xD8, 0xE2, 0xD7, 0x14, 0x0E, 0x0A, 0xCF, 0x0A, 0x0F, 0xF4, 0x06, 0xE7, 0x0C, 0x0A },
                age = 16
            },
            new Star {
                name = "Nhlanhla Shabalala",
                staffBytes = new byte[] { 0x12, 0x12, 0x10, 0x11, 0x0A, 0x08, 0x0F, 0x0C },
                playerBytes = new byte[] { 0x4A, 0x14, 0x00, 0x00, 0x00, 0x28, 0x00, 0x98, 0x00, 0xE8, 0x03, 0xE8, 0x03, 0xF4, 0x01, 0x01, 0x01, 0x01, 0x0A, 0x14, 0x0F, 0x01, 0x01, 0x01, 0x01, 0x14, 0x0F, 0x0D, 0x0C, 0x11, 0xDE, 0x0F, 0x0C, 0x10, 0x07, 0xC1, 0x22, 0x08, 0x50, 0xDE, 0x12, 0x0A, 0x88, 0xCA, 0x0C, 0x0F, 0x03, 0x0D, 0x0A, 0xBB, 0xBB, 0xDE, 0x12, 0x28, 0x10, 0xD9, 0xCA, 0xDE, 0xD0, 0x14, 0x12, 0x0E, 0xBB, 0x10, 0x0D, 0xF6, 0x0C, 0x13, 0x10, 0x0A },
                age = 16
            },
            new Star {
                name = "Ibrahim Said",
                staffBytes = new byte[] { 0x0F, 0x14, 0x14, 0x05, 0x11, 0x0D, 0x0E, 0x0A },
                playerBytes = new byte[] { 0xAB, 0x07, 0x00, 0x00, 0x00, 0x7D, 0x00, 0x7D, 0x00, 0x22, 0x24, 0x22, 0x24, 0xA0, 0x0F, 0x00, 0x14, 0x14, 0x10, 0x00, 0x00, 0x0A, 0x00, 0x00, 0x00, 0x14, 0x00, 0x0F, 0x10, 0x11, 0xE0, 0x0F, 0x12, 0x11, 0x08, 0xBC, 0xE5, 0x0E, 0xDB, 0xC1, 0x11, 0x0E, 0x88, 0xDB, 0x12, 0x0A, 0x11, 0x0E, 0x14, 0xBA, 0xE5, 0xD6, 0x12, 0x02, 0x0F, 0xD6, 0x1D, 0xE5, 0xD5, 0x14, 0x11, 0x0F, 0x1D, 0x10, 0x12, 0xF3, 0x10, 0xC3, 0x10, 0x0A },
                age = 21
            },
            new Star {
                name = "David Prutton",
                staffBytes = new byte[] { 0x10, 0x12, 0x12, 0x14, 0x0F, 0x0E, 0x0C, 0x0E },
                playerBytes = new byte[] { 0x56, 0x4D, 0x00, 0x00, 0x07, 0x87, 0x00, 0x93, 0x00, 0x7C, 0x15, 0x7C, 0x15, 0xDC, 0x05, 0x01, 0x01, 0x14, 0x14, 0x14, 0x01, 0x01, 0x0F, 0x14, 0x00, 0x14, 0x01, 0x0E, 0x0F, 0x0F, 0xF1, 0x0E, 0x0F, 0x10, 0x0C, 0xE3, 0xE8, 0x0C, 0xE3, 0xE3, 0x09, 0x0A, 0x88, 0xD4, 0x0E, 0x0C, 0x0C, 0x0F, 0x07, 0xE8, 0xEC, 0xF6, 0x10, 0xF2, 0x0F, 0xF1, 0xE3, 0xEC, 0xCA, 0x14, 0x0E, 0x0E, 0xEC, 0x12, 0x0F, 0x01, 0x12, 0xE8, 0x12, 0x0A },
                age = 19
            },
        };
    }
